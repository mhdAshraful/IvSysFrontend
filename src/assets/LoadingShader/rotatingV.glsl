...
float qinticInOut(float t) {
    return t < .5 ? +16. * pow(t, 5.) : -.5 * abs(pow(2. * t - 2., 5.)) + 1.;
}
...
// Therefore, the variable offset will represent the cosine of the angle between the  uAxis and newpos
    float offset = dot(normalize(uAxis), normalize(newPosition));

    // float distort = cnoise(vec3(normal));
    // float distort = cnoise(vec3(uDistortion, 0.04, offset));
...
// easing the progress 
    ...
    newPosition = rotate(newPosition, uAxis, distortonProgressOnOffset);
    vec3 newNormal = rotate(normal, uAxis, distortonProgressOnOffset);

    vNormal = normalMatrix * newNormal;
    vec4 worldPosition = modelMatrix * vec4(position, 1.);
    vReflectionTowardsCamera = normalize(worldPosition.xyz - cameraPosition);
